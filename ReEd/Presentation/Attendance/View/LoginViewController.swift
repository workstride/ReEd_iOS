...
class LoginViewController: UIViewController, UITextFieldDelegate {
    ...
    private let keepLoginStackView = UIStackView().then {
        $0.axis = .horizontal
        $0.spacing = 10
    }
    ...
    private let keepLoginLabel = UILabel().then {
        $0.text = "로그인 유지"
    ...
    private func setupSubviews() {
        keepLoginStackView.addArrangedSubview(checkBox)
        keepLoginStackView.addArrangedSubview(keepLoginLabel)
        
        [logoImage, emailTextField, passwordTextField, viewPasswordButton, loginButton, forgotPasswordbutton,keepLoginStackView].forEach {
            view.addSubview($0)
        }
    }
    ...
    private func setupConstraints() {
        ...
        keepLoginStackView.snp.makeConstraints {
        ...
        checkBox.snp.makeConstraints {
               $0.width.equalTo(20) // 체크박스의 고정된 너비
        }
           
        keepLoginLabel.snp.makeConstraints {
               $0.leading.equalTo(checkBox.snp.trailing).offset(0) // 체크박스 뒤에 간격을 둡니다.
        }
    }
    ...
    @objc private func didTapforgotPasdwordbutton() {
        dismiss(animated: true) {
            UIApplication.shared.windows.first?.rootViewController = InputEmailVC()
        }
    }
